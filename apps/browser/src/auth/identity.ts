...
/**
 * Best-effort revoke of an OAuth access token at Google. After this returns,
 * the user has to re-grant consent on the next `getAccessToken({interactive:true})`
 * call. Failure is non-fatal — the caller still clears local state.
 */
export async function revokeAccessTokenAtGoogle(token: string): Promise<boolean> {
	try {
